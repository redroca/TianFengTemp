...
@interface TFCycleBannerView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>

@property (strong, nonatomic) UIScrollView *scrollView;
@property (assign, nonatomic) BOOL scrollViewBounces;

@property (strong, nonatomic) TFPageControl *pageControl;
...
@implementation TFCycleBannerView
...
//    _pageControl = [[UIPageControl alloc] initWithFrame:pageControlFrame];
    _pageControl = [[TFPageControl alloc] initWithFrame:pageControlFrame indicatorMargin:5 indicatorWidth:5 currentIndicatorWidth:10 indicatorHeight:5];
    ...
    _pageControl.pageIndicatorColor = [[UIColor whiteColor] colorWithAlphaComponent:0.5];
    [self addSubview:_pageControl];
}

- (void)loadData {
//    NSAssert(_datasource != nil, @"datasource must not nil");
    _datasourceImages = [_datasource numberOfCycleBannerView:self];
    ...
    for (NSInteger i = 0; i < _datasourceImages.count; i++) {
        ...
        if ([imageSource isKindOfClass:[UIImage class]]) {
            ...
            [imageView requestImageDataURL:( [imageSource isKindOfClass:[NSString class]] ? [NSURL URLWithString:imageSource] : imageSource ) withPlaceholder:PlaceholderImgBigSquare];
        }
        [_scrollView addSubview:imageView];
    }
