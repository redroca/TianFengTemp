...
@interface TFNavigationBarManager ()
@property (nonatomic, strong) TFNavigationBar *selfNavigationBar;
...
@implementation TFNavigationBarManager
...
//                                                 forBarMetrics:UIBarMetricsDefault];
    [self sharedManager].selfNavigationBar.backgroundImage = image;
}
...
+ (void)setZeroAlphaOffset:(float)offset {
    ...
    [self setNavigationBarColorWithAlpha:offset];
}
...
//+ (void)reSetStoredNavigationBar {
//    if ([self sharedManager].storedNavigationBar) {
//        [[self sharedManager].selfNavigationController setValue:[self sharedManager].storedNavigationBar forKey:@"navigationBar"];
//    } else {
//        [self reStoreToSystemNavigationBar];
//    }
//}
...
//    UINavigationBar *navigationBar = viewController.navigationController.navigationBar;
    CGRect rect;
    if (IS_IPHONEX) {
        rect = CGRectMake(0, 44, SCREENWIDTH, 44);
    } else {
        rect = CGRectMake(0, 20, SCREENWIDTH, 44);
    }
    TFNavigationBar *navigationBar = [[TFNavigationBar alloc] initWithFrame:rect];
    
    
   
//    [viewController.view addSubview:navigationBar];
    [[UIApplication sharedApplication].keyWindow addSubview:navigationBar];
...
+ (void)changeAlphaWithCurrentOffset:(CGFloat)currentOffset {
    TFNavigationBarManager *manager = [self sharedManager];
...
+ (TFNavigationBarManager *)sharedManager {
    static TFNavigationBarManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[TFNavigationBarManager alloc] init];
        [self initBaseData:manager];
    });
    return manager;
}

+ (void)initBaseData:(TFNavigationBarManager *)manager {
